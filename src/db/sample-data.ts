...
/**
 * The demo outline, Time Chart and appointment a fresh test account starts with.
 *
 * Split out of `seed.ts` so `npm run user:create -- --sample-data` can load it for any
 * account, not only the one the seed happens to provision. Loading it is **destructive for
 * that user**: the whole tree, every appointment and every time chart go first.
 *
 * The content is Achieve's own manual examples (ACME account, Someday/Maybe, the ideal
 * week), which is deliberate — it doubles as a fidelity reference for the layout.
 */
...
/** Replace this user's outline, schedule and time charts with the demo content. */
