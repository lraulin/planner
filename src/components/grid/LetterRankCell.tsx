...
/**
 * A cell for a **hand-maintained ABCD ranking**, edited by typing.
 *
 * Deliberately not `PriorityCell`. That one stores what you type verbatim, including a bare
 * "A". Here a letter always resolves to a position, so what you type is a *request* that the
 * ranking rules answer: "A" lands at the end of A, "A2" inserts at position 2 and pushes the
 * rest down, and the cell then shows the rank you actually got. Clearing it drops the item
 * out of the ranking entirely.
 *
 * Shared by the Task Chooser's TC Priority and the Day tab's daily ABC — the same input
 * rules over two different fields, mirroring `src/lib/priority/letterRank.ts`, which shares
 * the placement rules underneath them.
 */
...
export function LetterRankCell({
  letter,
  rank,
  onAssign,
  ariaLabel,
  ...
  function commit() {
    ...
    setInvalid(false);
    if (parsed.letter === letter && parsed.rank === null) {
