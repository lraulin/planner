...
/** Searchable path for choosing the items a merge will consolidate, when a phone cannot Shift-click. */
export function SupplyMergePickerDialog({
  items,
  initiallySelected,
  onClose,
  onContinue,
  ...
  onClose: () => void;
  onContinue: (items: SupplyItemRow[]) => void;
}) {
  const titleId = useId();
  ...
  const [selected, setSelected] = useState(
    () =>
      new Set(
        [...initiallySelected].filter((id) => items.some((row) => row.id === id)),
      ),
  );
  const filtered = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase();
    if (!needle) return items;
    return items.filter((row) => row.name.toLocaleLowerCase().includes(needle));
  }, [items, query]);
    ...
    <ModalShell open onClose={onClose} labelledBy={titleId} width="max-w-lg">
      <div className="flex max-h-[min(42rem,calc(100dvh-2rem))] flex-col p-5">
        ...
        <p className="mt-2 text-[0.8125rem] leading-relaxed text-ink-muted">
          Choose at least two items. You will choose the survivor and review every moved
          offer next.
        </p>
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-[0.75rem] text-ink-muted">
                  ...
                  {item.options.length > 0
                    ? ` · ${item.options.length} offer${item.options.length === 1 ? "" : "s"}`
                    : ""}
                </span>
              </span>
            </label>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              onClick={() => onContinue(items.filter((item) => selected.has(item.id)))}
