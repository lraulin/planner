...
export function SuggestFromAmazonDialog({
  ...
  return (
    <ModalShell open onClose={onClose} labelledBy={headingId} width="max-w-3xl">
      <h2 id={headingId} className="mb-1 text-[0.9375rem] font-semibold text-ink">
        Suggest from Amazon
      </h2>
      <p className="mb-3 text-[0.8125rem] text-ink-muted">
        Things you have ordered more than once, or on Subscribe &amp; Save. The rate is
        estimated from how much you bought over how long — correct it in the worksheet.
      </p>

      {error ? <p className="mb-3 text-[0.8125rem] text-priority-a">{error}</p> : null}
          ...
          rows.map((row) => (
            ...
            >
              ...
              <button
                ...
                className="min-h-tap shrink-0 rounded border border-rule px-3 text-[0.8125rem] text-ink hover:bg-surface-raised disabled:text-ink-faint md:min-h-0 md:py-1.5"
                onClick={() => add(row)}
              >
