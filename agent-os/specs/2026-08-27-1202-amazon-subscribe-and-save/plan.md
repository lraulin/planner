...
| 3   | Order money moves out of this spec: an order stores Amazon's printed summary and totals, and orders are enumerated from order history rather than a payments walk. See `agent-os/specs/2026-08-27-1521-amazon-order-totals-register-link/` | Task 8 verification found the Group by Order total is the item-line sum, which is not what the card is charged. Order `111-7959899-2189857` sums to $23.49 against a $23.66 charge: a −$1.17 subscription saving and $1.34 tax were never captured and `amazon_orders` had no column to hold a total. The dropped discount is the S&S value this spec exists to track, so its acceptance criteria cannot pass until the delta lands. Both specs freeze together |
