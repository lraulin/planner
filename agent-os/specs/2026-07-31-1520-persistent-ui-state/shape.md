...
- De-duplicated common column definitions
...
- Saved named views, a Views & Filters sidebar, a custom filter builder — all listed under
  "Out of roadmap (for now)" (`agent-os/product/roadmap.md:303`)
...
- Column add / remove / rearrange **already exists** (`useGridColumns.ts` +
  `ShowFieldsDialog.tsx`) — it needs to move stores and gain widths, not be built.
- Header sort **already exists** on four tabs but is **broken under grouping**
  (`DataGrid.tsx:196-213` skips the sort while still drawing the arrow).
- Filter state is **private to `DataGrid`** (`DataGrid.tsx:122-123`), which is the single fact
  blocking persistence, a clear-filters button, and shareable state.

- **Visuals:** none. The work is behavioural, against existing UI.
- **References:** see `references.md`.
- **Product alignment:** delivers the Phase 1 "light polish on the main grids" line
  (`agent-os/product/roadmap.md:65`) and the "Show Fields move up/down is coarse" caveat at
  `roadmap.md:36-41`. Serves the mission's "own your data" and multi-user-ready posture by
  putting preferences where the rest of the user's data already lives.

## Standards Applied

- `database/migrations` — a new table; generate the migration, never hand-write it
- `development/testing` — pure logic in `src/lib/**` with adjacent tests; the new mutations
  need an `*.integration.test.ts` with the cross-user case; no React component tests
