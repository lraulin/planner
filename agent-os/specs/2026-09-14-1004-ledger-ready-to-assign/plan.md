...
| 5   | `descriptionsOverlap` (`liveFeedMatch.ts`) gained a fifth, narrow rule: both sides matching known payment-posting wording (`Payment from X`, `X ONLINE/MOBILE PYMT`, `PAYMENT THANK YOU`, …) counts as overlap, feeding `pairRows` for every retirement, not only `resolveLostHold`'s hold-vanished case. This is **not** a reversal of row 3: row 3 rejected giving `pairRows` `resolveLostHold`'s _single-qualifying-candidate_ leniency, which breaks occurrence-counted duplicates; this instead widens what counts as description overlap, leaving the existing batch-matching/occurrence-counting machinery untouched. | Fix-bug: a card's own pending page names an account payment by source (`Payment from CAPITAL ONE N.A. ...2322`); SimpleFIN names the same payment by channel (`CAPITAL ONE ONLINE PYMT`). Neither containment nor the brand stem ever bridges that, so the browser row never retired and permanently inflated the budget's unmatched-transfer total. |
