...
- **Supersedes:** `2026-09-01-1205-source-as-of-authority` **D3** (pending authority by comparing
  the capture's stamp with the feed's): whose pending holds count now follows the account's history
  source (Changes row 14).
...
| 13  | A parsed row's `description` stays the page's display name (identity and planner matching unchanged) and carries `statementDescriptor` beside it; only the write puts the descriptor in `description` and the name in `bank_display_name`, and stored rows reach the planner as `bank_display_name ?? description`.  | A posted row stored with its descriptor must still be recognised by the next paste by name: a closed statement's copy gets a new external id, and a descriptor like "GOOGLE *GOOGLE PLAY" shares nothing with "YouTube".                                                                                                                                              |
| 14  | Whose pending holds count follows `history_source` (`bank_page` → the page's, else the feed's), replacing the stamp comparison in `workingPending.ts`. With it go the sync's suppression of SimpleFIN holds while a capture was newer, the Accounts page's "Paste fresh snapshot" warning, and `browserOwnsPending`. | Found verifying the Ready-to-Assign criterion: after cutover the envelope carried onto the page's hold vanished from the budget, because with no capture newer than SimpleFIN's balance the timestamp rule still counted SimpleFIN's holds. It was a second read-time guess at who owns the account; production only worked because Lee's paste happened to be newer. |
...
## Task 4: Userscript and parser: statement descriptor **done**
