# Standards for Zero-based budgeting
...
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `development/clean-code`   | The envelope math is pure `src/lib/**`; actions stay thin; every mutation takes `userId`                                                                  |
| `development/testing`      | Pure math gets a unit test; every DB mutation gets an integration test with a second user; no React component tests; `npm run smoke` after adding a route |
| `development/security`     | Four new user-scoped tables — a dropped `userId` is invisible with one user                                                                               |
| `development/dates`        | Month keys are calendar values; `startOfDay` on one is the Aug 1 → Jul 31 regression                                                                      |
| `development/commits`      | The record of why envelopes were reopened lives in the commit body                                                                                        |
| `database/migrations`      | Four new tables and two column adds                                                                                                                       |
| `components/data-grid`     | The group → category tree, persisted view state, the row menu                                                                                             |
| `components/ux-principles` | Inline assignment editing, decimal commit on blur                                                                                                         |
| `components/navigation`    | Registering `/finances/budget` and its commands                                                                                                           |
