...
- [x] `/library/houses` page renders a grid + drawer following the standard pattern
- [x] A house can be created, edited, and deleted; a second user cannot read/change/delete it
- [x] Drive time and distance to 5006 Valley Drive, Chesapeake Beach, MD 20732 are computed
      automatically on save when the address changes, cached on the row, and retryable from the
      row menu on failure
- [x] Priority (standard ABCD/rank) and status (Available/Not Interested/No Longer
      Available/Offer Made, default Available) are set from the grid/drawer
- [x] Grok can list, read, create, update, and delete houses over MCP (`houses` domain)
- [x] `npm run lint && npm run typecheck && npm test` pass, including the cross-user integration
...
| 2   | Priority column renders plain text, not `priorityColumn()`/`LetterRankCell`                                                                                                      | `LetterRankCell` is inline-editable, contradicting "every column read-only" one line above it in the same plan section; used the Metrics precedent (plain text, same flat-catalog shape) instead                                                                                                                                                                                                                                                                                                                                                                                                                |
| 3   | `houses/mutations.ts` resolves priority through `letterRank.ts`'s `letterRankEngine`, not a raw column patch                                                                     | Found while wiring the MCP tools: a bare letter with no rank (typing "A" in `PriorityField`, or `create_house`/`update_house` supplying one without the other) wrote the letter with a stale/null rank and violated `houses_priority_letter_ranked`. Houses is a flat pool like the Task Chooser or Day list, where `assertRankedLetterPriorities`'s rule applies — a bare letter must resolve to a real position, the way the Outline's own detail-save already does via `assignPriorityAmongSiblings`. Metrics has the same latent gap in its own raw patch; out of scope here since Metrics has no spec open |
| 4   | `houses.state`/`houses.status` get their own zod `.describe()` in the MCP contracts                                                                                              | Both field names already exist in `tools.ts`'s global `fieldDescriptions` map with a different meaning (outline lifecycle state; subscription status) — the map has no per-tool scoping, so Houses' schemas need an explicit description to not inherit the wrong one                                                                                                                                                                                                                                                                                                                                           |
| 5   | Added `DecimalField` to `detail/fields.tsx`                                                                                                                                      | Baths (`numeric(3,1)`) and lot acreage (`numeric(6,3)`) need a decimal-string field the kit didn't have; generalized `MoneyField`'s pattern (configurable decimal count, no currency stripping) rather than hand-rolling a local one-off                                                                                                                                                                                                                                                                                                                                                                        |
| 6   | Fixed four pre-existing hardcoded MCP tool-count assertions unrelated to Houses (`pages.test.ts`'s Library page list, `mcp.test.ts` × 2, `route.test.ts`, `smoke-agent.mjs` × 2) | Adding a fifth tool domain changed the total tool count from 50 to 55 and the Library page list gained an entry; every hardcoded count in the suite had to move together or `npm test`/`npm run smoke:agent` would fail on unrelated files. `smoke-agent.mjs`'s finance-domain check was separately stale (asserted 6 tools; there are 12) — caught while fixing the others, not caused by this work                                                                                                                                                                                                            |
