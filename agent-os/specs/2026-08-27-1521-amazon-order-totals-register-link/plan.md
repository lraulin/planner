...
- [ ] Order `111-7959899-2189857` stores Item(s) subtotal $23.49, Subscription saving −$1.17,
      ...
      charge.
- [ ] An order whose summary lines do not reconcile to its grand total is flagged, not silently
      summed.
- [ ] A multi-shipment order records several charges and links to each matched bank row.
- [ ] A mixed order allocates the subscription saving only across its S&S lines, and tax across
      all lines; children still balance to the cent.
- [ ] A `TOTAL $0.00` gift-card-funded order imports without inventing a card charge.
- [ ] A privacy-zip order gets a derived summary marked `derived`, never `printed`.
- [ ] Re-pasting is a no-op for settled evidence and never overwrites a manual split, a Bill
      name/group, a reviewed mapping or an existing Supply envelope.
- [ ] A v1 snapshot paste is refused with an actionable message.
- [ ] Every new query and mutation is user-scoped; a second user cannot read, change or delete
      the first user's order totals or charge links.
- [ ] Desktop and compact Orders expose the same totals, columns and review actions.
...
- [ ] Add the `amazon_orders` summary columns and generate, inspect and apply the Drizzle
      migration with snapshot and journal entry.
- [ ] Add a pure reconciliation module: parse labeled summary lines to cents, classify the
      known labels, keep unknown labels verbatim, and report whether the order reconciles.
- [ ] Cover the C4 order, a gift-card `$0.00` order, an unknown extra line and a
      does-not-reconcile order in unit tests.

## Task 3: Snapshot v2 and the userscript

- [ ] Extend the snapshot types with order summaries and per-order charges; reject v1.
- [ ] Rewrite the userscript's order-history walk, order-detail extraction (`chargeSummary`
      and the `data-component` item fields) and per-order transactions fetch.
- [ ] Pin malformed money, missing summary blocks, duplicate lines and partial pages in tests.

## Task 4: Persist and derive

- [ ] Store printed summaries from the capture and derived summaries from the zip import.
- [ ] Give charges the stable natural key and keep charge↔order links additive.
- [ ] Cover upsert idempotence, key stability across a re-capture, and cross-user
      read/change/delete refusal in integration tests.

## Task 5: Allocation correction

- [ ] Allocate tax across all lines and the subscription saving across S&S lines only.
- [ ] Keep children balanced to the cent and manual splits protected.

## Task 6: Order ↔ register surface

- [ ] Derive the order's register link from its charges' matches.
- [ ] Add review-only manual order↔row linking on equal grand totals.
- [ ] Add the `Order total` and `Register` columns and the reconciliation marker; show the
...
- [ ] Real capture → paste → confirm the C4 order reconciles to $23.66 and links to its
      register row; check desktop, 390×844 and both colour schemes.
- [ ] Verify the S&S spec's own acceptance criteria, then mark **both** specs
      **frozen / complete** and update the roadmap.
