...
| 10  | A full plain-monthly bill amount in Activity satisfies that month's derived target even if the claimed-payee recurrence anchor has not advanced.                                                                                      | Recurrence identity and envelope funding are different facts. Claimed payees still own the next-charge date, preserving `2026-08-25-0901-bill-next-charge` D4, while an assigned-and-spent bill no longer asks for the same occurrence twice after a manual Category placement.                  |
| 11  | Adding a payee alias immediately recomputes transaction payee identities and files newly recognized uncategorized rows through the payee's claim or default.                                                                          | `payee_id` is derived from aliases, but the drawer changed only the alias row. The Payees page could therefore show `RENT:RAULIN` under Rent while recurrence still read a stale/null transaction pointer until a later import reclassified the ledger.                                          |
