...
- **D5 — Resolution is deterministic and conflicts stop the whole user.** Resolve each
  ...
  unresolved references, or a parity difference is reported before any write. The executor
  runs in one transaction scoped to one user and is idempotent.
...
- [ ] Dashboard, Available to Spend, Insights, Sankey, review, commitment rates and every
      other finance figure are byte-identical before and after the cutover.
