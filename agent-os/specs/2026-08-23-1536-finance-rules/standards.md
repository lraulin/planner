...
The following standards apply to this work. They are copied in full so the active
...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `database/migrations`       | A new `finance_rules` table plus a seeded data migration over the real file.                                                                                                                |
| `development/clean-code`    | `payees/seed.ts` stops importing from `classify/`; preview reuses `planReclassify` + `changedRows` instead of reimplementing a run; per-field rule composition is deliberately _not_ built. |
| `development/testing`       | Pure matching logic with sibling tests; every mutation with a cross-user case; no component tests.                                                                                          |
| `development/security`      | Every mutation takes `userId` and proves ownership; one user's rules must never reach another's rows.                                                                                       |
| `development/commits`       | The refund rekey is its own logical change, shipped and verified before the rules table.                                                                                                    |
| `components/ux-principles`  | Grid + drawer, inline editing, a modal only for the preview confirmation.                                                                                                                   |
| `components/navigation`     | A command without a menu entry is not shipped; unavailable is disabled with the specific reason.                                                                                            |
| `components/data-grid`      | Drag-to-reorder is first-class here because the order _is_ the priority model.                                                                                                              |
| `components/drawer-pattern` | The rule editor is a drawer with the standard footer.                                                                                                                                       |
| `components/modal-pattern`  | The run preview is a confirm-then-apply `ModalShell`.                                                                                                                                       |
| `components/responsive`     | Drag is disabled on touch, so the phone `⋯` menu must carry Move up / Move down.                                                                                                            |
