...
| 5   | A Coinbase Sell that precedes a USD withdrawal is stored at the bank-received Subtotal, not Coinbase's Sell Total.                                            | Pairing withdrawal ↔ checking as `internal_transfer` would otherwise delete the liquidation from cash flow. Giving the Sell the same dollars the bank received makes Coinbase's two USD legs cancel and leaves the sale visible as `external_transfer`. The $8.59 fee on the Nov 2025 row is the case that made Total the wrong figure.                     |
