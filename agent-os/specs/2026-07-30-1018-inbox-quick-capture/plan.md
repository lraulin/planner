...
`db:generate`** — `drizzle/meta/` is drifted (0007's snapshot is a byte-identical copy of
0003), so generation prompts to re-apply 0004–0007. Two prior specs hit this and both
hand-wrote; snapshot repair remains a separate follow-up.
...
| **Repair `drizzle/meta` snapshots**        | Still drifted — `0007_snapshot.json` is a byte-identical copy of `0003`, so `db:generate` remains unusable and every migration since `0004` has been hand-written                                                                                                                                                                                          |
