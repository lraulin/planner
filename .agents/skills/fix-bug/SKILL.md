---
name: fix-bug
description: Investigate a reported bug down to its root cause before fixing it, and size the fix deliberately. Use whenever an error, a crash, a failing test, or behavior that is wrong is reported — and when the user invokes /fix-bug or $fix-bug.
---
