...
description: Select and load the Planner coding standards relevant to the work at hand. Use before writing or reviewing non-trivial code, when authoring a skill or shaping a spec, or when the user invokes /inject-standards or $inject-standards. Load only what the task needs.
---
...
2. **Creating a Skill** — Building a skill under `.agents/skills/` (the canonical home; see
   `AGENTS.md`) or a harness-specific equivalent such as `~/.grok/skills/`
...
- If conversation clearly mentions creating a skill, editing `.agents/skills/`, or
  building a reusable procedure → **Creating a Skill**
...
- `.agents/skills/` — the canonical home for every project workflow. The other locations
  are thin pointers into it, so this is the only directory worth scanning.
