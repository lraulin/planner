---
name: overnight
description: Unattended self-directed work — pick useful work, verify it, commit it, and leave a morning report, asking no questions. Use ONLY when the user explicitly invokes /overnight or $overnight. Never infer it from context and never start it on your own.
---
...
**Check for the skip warning.** `npm run test:unit` passing does _not_ mean the database
...
- Harness-agnostic: this file is the canonical workflow. Claude Code and Grok reach it
  through the shim at `.claude/commands/overnight.md`, Copilot through
  `.github/prompts/overnight.prompt.md`, and Codex reads it directly as `$overnight`.
- **Explicit invocation only.** Unlike the other workflows here, this one never starts on
  its own — `allow_implicit_invocation` is `false` and the skill description forbids
  inferring it. Nobody wants an unattended commit spree they did not ask for.
